angular.module('starter.discoverMap', [])
  .controller('DiscoverCtrl', function ($scope, $rootScope, $stateParams, IMAGE_PATH, $state, $ionicNavBarDelegate, $ionicModal, $cordovaDatePicker, $ionicSideMenuDelegate, $timeout, APIService, STYLE_JSON,ionicToast) {
    $scope.isMap = true;
    $scope.search = "";
    ...
    $scope.getSeacrh = function (isloader, serverCall, resetMarker) { //reload marker property
    ...
    function reloadMarkers(data) { //data.length == 0 &&
      ...
      if ($scope.isMarketRefresh && angular.element(document.getElementById('disconver-input'))[0] && angular.element(document.getElementById('disconver-input'))[0].value) {
        ...
        let objData = {};
        objData.search = angular.element(document.getElementById('disconver-input'))[0].value;
        objData.type = ribbonFilter();
        APIService.getSearchCount(objData).then(function (response) {
            if (response.data.total_count == 0) {
              $scope.toastShortBottomLong(`'${response.data.total_count}' results found. Change your search result.`);
            } else {
              $scope.toastShortBottomLong(`'${response.data.total_count}' results found. Zoom out to see all results.`);
            }
          },
          function (error) {
    ...
    $scope.supportedType = function (data) {
      ...
      let index = $scope.listData.indexOf($scope.listData.filter(function (i) {
