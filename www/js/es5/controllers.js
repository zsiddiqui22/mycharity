...
angular.module('starter.controllers', []).directive('ngEnter', function () {
  ...
  return {
    link: function link(scope, element, attrs) {
      attrs.$observe("autoFocus", function (newValue) {
        if (newValue === "true") $timeout(function () {
          element[0].focus();
        });
      });
    }
...
// live URL
.value('BASE_URL', 'https://mycharityhero.co.uk/MyCharityHero/public/api').value('IMAGE_PATH', 'https://mycharityhero.co.uk/MyCharityHero/public/images/').value('ATTACH_PATH', 'https://mycharityhero.co.uk/MyCharityHero/public/attachments/').value('STYLE_JSON', [{
  ...
  controller: function controller() {//component controller
  }
  ...
  $scope.imagePath = IMAGE_PATH; // document.addEventListener('deviceready', function () {
  ...
  $scope.showPopup = function (title, message, txt_btn_cancel, txt_btn_submit, redirect_url) {
    var url = redirect_url; // Custom popup
    ...
    var myPopup = $ionicPopup.show({
      ...
      scope: $scope,
      buttons: [{
        text: "<b>".concat(txt_btn_cancel, "</b>"),
        type: 'button-stable',
        onTap: function onTap(e) {}
      }, {
        text: "<b>".concat(txt_btn_submit, "</b>"),
  ...
  $scope.toastShortCenter = function (content) {
    if (ionic.Platform.platform() == 'ios' || ionic.Platform.platform() == 'android') {
      alert(content);
      ionicToast.show(content, 'middle', false, 1000); // $cordovaToast.showShortCenter(content).then(function (success) {
      //   // success
      //   console.log('Success');
      // }, function (error) {
      //   // error
  ...
  $scope.toastLongCenter = function (content) {
    if (ionic.Platform.platform() == 'ios' || ionic.Platform.platform() == 'android') {
      ionicToast.show(content, 'middle', false, 2000); // $cordovaToast.showLongCenter(content).then(function (success) {
      //   // success
      // }, function (error) {
      //   // error
      // });
  ...
  $scope.toastShortTop = function (content) {
    if (ionic.Platform.platform() == 'ios' || ionic.Platform.platform() == 'android') {
      ionicToast.show(content, 'top', true, 1000); // $cordovaToast.showShortTop(content).then(function (success) {
      //   // success
      // }, function (error) {
      //   // error
      // });
  ...
  $scope.toastLongBottom = function (content) {
    if (ionic.Platform.platform() == 'ios' || ionic.Platform.platform() == 'android') {
      ionicToast.show(content, false, 2000); // $cordovaToast.showLongBottom(content).then(function (success) {
      //   // success
      // }, function (error) {
      //   // error
      // });
  ...
  $scope.toastLongTop = function (content) {
    if (ionic.Platform.platform() == 'ios' || ionic.Platform.platform() == 'android') {
      ionicToast.show(content, 'top', true, 2000); // $cordovaToast.showLongTop(content).then(function (success) {
      //   // success
      // }, function (error) {
      //   // error
      // });
  ...
  $scope.toastShortBottom = function (content) {
    if (ionic.Platform.platform() == 'ios' || ionic.Platform.platform() == 'android') {
      ionicToast.show(content, 'bottom', false, 1000); // $cordovaToast.showShortBottom(content).then(function (success) {
      //   // success
      // }, function (error) {
      //   // error
      // });
  ...
  $scope.toastShortBottomLong = function (content) {
    if (ionic.Platform.platform() == 'ios' || ionic.Platform.platform() == 'android') {
      ionicToast.show(content, 'bottom', true, 1000); // $cordovaToast.showShortBottom(content).then(function (success) {
      //   // success
      // }, function (error) {
      //   // error
      // });
  ...
  $scope.logout = function () {
    ...
    $state.go('app.discover'); // call main emit function
  ...
  $scope.submit = function () {
    ...
    APIService.login(obj).then(function (response) {
      ...
      $rootScope.user_name = JSON.parse(localStorage.mch_mob_data).name;
      $scope.hideLoader();
      $scope.gotoDiscover(); // $state.go('app.discover',{reload: true, inherit: false});
    }, function (error) {
  ...
  $scope.submit = function () {
    ...
    APIService.forgotPass(obj).then(function (response) {
      $scope.toastShortCenter(response.data.message); // $scope.toastShortCenter('Record saved successfully');
  ...
  $scope.gotoCreate = function () {
    if (JSON.parse(localStorage.mch_mob_data).role === 'user') {
      ...
      $scope.showGuestPopup(); // $scope.toastShortCenter('Must have login first');
    }
  };
  ...
  $ionicModal.fromTemplateUrl('templates/create.html', {
    ...
    $scope.openDatePicker = function () {
      var calanderObj = {
        ...
        disabledDates: [// new Date(2019, 2, 1),
        ],
  ...
  if ($state.current.name === 'app.organzationcedit') {
    ...
    APIService.getProfile($stateParams.id).then(function (response) {
      ...
      response.data.attachments.forEach(function (el) {
  ...
  $ionicModal.fromTemplateUrl('templates/googlemap.html', {
    ...
    $scope.initMap = function () {
      ...
      var objlatlng = new window.google.maps.LatLng(location.lat, location.lng);
      $scope.geoCoderReverse(objlatlng); //new end
      ...
      google.maps.event.addListener(marker, 'dragend', function (evt) {
        marker.setVisible(false);
        marker.setPosition(evt.latLng); // const latlng = {
        //   lat: evt.latLng.lat(),
        //   lng: evt.latLng.lng()
        // }
      ...
      autocomplete.addListener('place_changed', function () {
        ...
        marker.setVisible(true); // const latlng = {
        //   lat: place.geometry.location.lat(),
        //   lng: place.geometry.location.lng()
        // }

        $scope.geoCoderReverse(place.geometry.location); // $scope.geoCoderReverse(place.geometry.location);
      });
    };
    ...
    $scope.currentLocation = function () {
      if (navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition(function (position) {
          ...
          // };
          var objlatlng = new window.google.maps.LatLng(position.coords.latitude, position.coords.longitude);
          marker.setVisible(false);
  ...
  $scope.imagePath = IMAGE_PATH; //first start
  ...
  $scope.supportedType = function (data, objname) {
    if (objname == 'myOrganization') {
      var index = $scope.myOrganization.indexOf($scope.myOrganization.filter(function (i) {
        return i.id == data.id;
      })[0]);
      $scope.myOrganization[index].supported = true;
    } else if (objname == 'supported') {
      var _index = $scope.supported.indexOf($scope.supported.filter(function (i) {
        return i.id == data.id;
      })[0]);

      $scope.supported[_index].supported = true;
    }
  ...
  $scope.usSupportedType = function (data, objname) {
    if (objname == 'myOrganization') {
      ...
      var _index2 = $scope.supported.indexOf($scope.supported.filter(function (i) {
        return i.id == data.id;
      })[0]);

      $scope.supported.splice(_index2, 1);
    }
  ...
  $scope.supportedType = function (data, objname) {
    if (objname == 'suggested') {
      var index = $scope.suggested.indexOf($scope.suggested.filter(function (i) {
        return i.id == data.id;
      })[0]);
  ...
  $scope.usSupportedType = function (data, objname) {
    if (objname == 'suggested') {
      ...
      $scope.suggested[index].supported = false; // $scope.suggested.splice(index, 1);
    }
  ...
  $scope.supportedType = function (data, objname) {
    if (objname == 'past') {
      var index = $scope.past.indexOf($scope.past.filter(function (i) {
        return i.id == data.id;
      })[0]);
  ...
  $scope.usSupportedType = function (data, objname) {
    if (objname == 'past') {
      ...
      $scope.suggested[index].supported = false; // $scope.past.splice(index, 1);
    }
  ...
  $scope.showGuestPopup = function () {
    ...
    }, function (msg) {// console.log('message:', msg);
    });
  };
  ...
  if ($state.current.name === 'app.projectedit') {
    ...
    APIService.getProfileProject($stateParams.id).then(function (response) {
      ...
      $scope.create = (_$scope$create = {
        ...
        longitude: response.data.longitude,
        id: $stateParams.id
      }, _defineProperty(_$scope$create, "funds_target", parseInt(response.data.funds_target)), _defineProperty(_$scope$create, "currency", response.data.currency), _defineProperty(_$scope$create, "attachments", []), _$scope$create);
      response.data.attachments.forEach(function (el) {
        $scope.create.attachments.push([ATTACH_PATH + el.name, el.type]);
      });
    }, function (error) {
    ...
    if (form.$valid) {
      ...
      if (obj.id) {
        ...
        APIService.addProject(obj).then(function (response) {
          ...
          $timeout(function () {
            defaultPopModal();
            var pac_input = document.getElementById('pac-input-project');
  ...
  if ($state.current.name === 'app.heroedit') {
    ...
    APIService.getHeroProfile($stateParams.id).then(function (response) {
      ...
      $scope.create = (_$scope$create2 = {
        ...
        longitude: response.data.longitude,
        id: $stateParams.id
      }, _defineProperty(_$scope$create2, "funds_target", parseInt(response.data.funds_target)), _defineProperty(_$scope$create2, "attachments", []), _$scope$create2);
      response.data.attachments.forEach(function (el) {
        $scope.create.attachments.push([ATTACH_PATH + el.name, el.type]);
      });
    }, function (error) {
  ...
  $scope.submit = function () {
    ...
    if (obj.id) {
      ...
      APIService.addHero(obj).then(function (response) {
        ...
        $timeout(function () {
          defaultPopModal();
          var pac_input = document.getElementById('pac-input-hero');
  ...
  $scope.supportedType = function (data, objname) {
    if (objname == 'myOrganization') {
      ...
      var _index3 = $scope.supported.indexOf($scope.supported.filter(function (i) {
        return i.id == data.id;
      })[0]);

      $scope.supported[_index3].supported = true;
    }
  ...
  $scope.usSupportedType = function (data, objname) {
    if (objname == 'myOrganization') {
      ...
      var _index4 = $scope.supported.indexOf($scope.supported.filter(function (i) {
        return i.id == data.id;
      })[0]);

      $scope.supported.splice(_index4, 1);
    }
    ...
    var timeObj = {
      callback: function callback(val) {
        if (typeof val === 'undefined') {
          ...
          if (arg == 'starttime') {
            $scope.create.starttime = "".concat(selectedTime.getUTCHours(), " : ").concat(selectedTime.getUTCMinutes());
          } else {
            $scope.create.endtime = "".concat(selectedTime.getUTCHours(), " : ").concat(selectedTime.getUTCMinutes());
          }
        }
      },
  ...
  if ($state.current.name === 'app.challangeedit') {
    ...
    APIService.getProfileChallenge($stateParams.id).then(function (response) {
      ...
      $scope.create = (_$scope$create3 = {
        ...
        longitude: response.data.longitude,
        id: $stateParams.id
      }, _defineProperty(_$scope$create3, "funds_target", parseInt(response.data.funds_target)), _defineProperty(_$scope$create3, "currency", response.data.currency), _defineProperty(_$scope$create3, "attachments", []), _$scope$create3);
    ...
    if (form.$valid) {
      ...
      if (obj.id) {
        ...
        APIService.addChallenge(obj).then(function (response) {
          ...
          $timeout(function () {
            defaultPopModal();
            var pac_input = document.getElementById('pac-input-challenge');
  ...
  $ionicModal.fromTemplateUrl('templates/googlemap.html', {
    ...
    $scope.initMap = function () {
      ...
      var input = document.getElementById('pac-input-challenge'); // input.value = $scope.create.orgAddress;
  ...
  $scope.supportedType = function (data, objname) {
    if (objname == 'myOrganization') {
      ...
      var _index5 = $scope.supported.indexOf($scope.supported.filter(function (i) {
        return i.id == data.id;
      })[0]);

      $scope.supported[_index5].supported = true;
    }
  ...
  $scope.usSupportedType = function (data, objname) {
    if (objname == 'myOrganization') {
      ...
      var _index6 = $scope.supported.indexOf($scope.supported.filter(function (i) {
        return i.id == data.id;
      })[0]);

      $scope.supported.splice(_index6, 1);
    }
  ...
  $scope.supportedType = function (data, objname) {
    if (objname == 'heros') {
      var index = $scope.heros.indexOf($scope.heros.filter(function (i) {
        return i.id == data.id;
      })[0]);
  ...
  $scope.supportedType = function (data, objname) {
    if (objname == 'celebrity') {
      var index = $scope.celebrity.indexOf($scope.celebrity.filter(function (i) {
        return i.id == data.id;
      })[0]);
  ...
  function stripeResponseHandler(status, response) {
    if (status != 200) {
      $scope.hideLoader();
      $scope.toastShortCenter(response.error.message); // globalService.showAlert('ds', "Ops someting where wrong");
    } else {
      var obj = {};
      ...
      obj.card_save = true; // obj.last_four = $scope.creditcard.number.substring($scope.creditcard.number.length - 4);
      ...
      APIService.addPayment(obj).then(function (response) {
        ...
        $scope.toastShortCenter(error_response);
        $scope.hideLoader();
        alert(error_response.data); // globalService.showAlert('ds', error_response.data);
      });
    }
  }
});