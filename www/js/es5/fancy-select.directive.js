...
function fancySelectDirective($rootScope, $parse, $timeout, genericModalService) {
  return {
    ...
    link: function link(scope, element, attrs, ngModelCtrl) {
      element.addClass('fancy-select'); // Validate directive attributes.
      ...
      function showItems() {
        ...
        if (multiSelect && typeof modalScope.selection.selected !== 'undefined' && Array.isArray(modalScope.selection.selected)) {
          ...
          modalScope.selection.selected.forEach(function (item) {
            ...
            var itemIndex = modalScope.selection.selectable.findIndex(function (x) {
...
function fancySelectTemplate($templateCache) {
  $templateCache.put('template/fancy-select/fancy-select.html', '<ion-modal-view class="fancy-select-modal">' + '	<ion-header-bar align-title="center" class="bar-positive">' + '		<button class="button button-clear button-icon ion-arrow-left-c" ng-click="closeModal()"></button>' + '		<h1 class="title">{{headerText}}</h1>' + '		<button class="button button-clear button-icon ion-checkmark-round" ng-if="!closeOnSelection" ng-click="select(); closeModal();"></button>' + '	</ion-header-bar>' + '	<ion-content>' + '       <div class="list">' + '           <div class="item item-input-inset">' + '         <label class="item-input-wrapper">' + '               <input type="text" placeholder="Search" ng-model="searchText">' + '         </label>' + '           <button class="button button-small">' + '              Clear' + '           </button>' + '           </div>' + '       </div>' + '		<div ng-if="group">' + '			<div class="list cus-lable" ng-if="multiSelect" ng-repeat="(key, value) in selection.selectable | groupBy : group | filter:searchText">' + '				<ion-item ng-if="groupLabel" class="item-divider">{{groupLabel(value[0])}}</ion-item>' + '				<ion-checkbox ng-repeat="item in value" ng-model="item.selected">{{itemLabel(item)}}</ion-checkbox>' + '			</div>' + '			<div class="list cus-lable" ng-if="!multiSelect" ng-repeat="(key, value) in selection.selectable | groupBy : group | filter:searchText">' + '				<ion-item ng-if="groupLabel" class="item-divider">{{groupLabel(value[0])}}</ion-item>' + '				<ion-radio ng-repeat="item in value" ng-model="selection.selected" ng-value="item" name="fancy-select">{{itemLabel(item)}}</ion-radio>' + '			</div>' + '		</div>' + '		<div ng-if="!group">' + '			<div class="list cus-lable">' + '				<ion-checkbox ng-if="multiSelect" ng-repeat="item in selection.selectable | filter:searchText" ng-model="item.selected">{{itemLabel(item)}}</ion-checkbox>' + '				<ion-radio ng-if="!multiSelect" ng-repeat="item in selection.selectable | filter:searchText" ng-model="selection.selected" ng-value="item" name="fancy-select">{{itemLabel(item)}}</ion-radio>' + '			</div>' + '		</div>' + '	</ion-content>' + '</ion-modal-view>');
}

angular.module('starter').run(fancySelectTemplate).directive('fancySelect', fancySelectDirective).directive("donationButton", function () {
  var controller = ['$scope', '$ionicModal', 'APIService', '$stateParams', '$ionicLoading', 'ionicToast', '$timeout', function ($scope, $ionicModal, APIService, $stateParams, $ionicLoading, ionicToast, $timeout) {
    ...
    $scope.alreadyCard = function (isbanktransfer) {
      ...
      obj.pay_more = [];
      obj.total_amount = 0;
      $scope.donate.paymore.forEach(function (element) {
        var objCard = {};
      ...
      obj.gift_aid = $scope.donate.gift_aid; // obj.total_amount = $scope.donate.amount;
    ...
    $ionicModal.fromTemplateUrl('templates/donataion.html', {
      ...
      $scope.modal = modal;
      $scope.defaultPopModal(); // $scope.getCards();
  ...
  return {
    ...
    link: function link($scope, element, attrs) {
      $scope.showDonation = function () {
        ...
        var objData = $scope.$eval(this.data);
        $scope.donate.id = objData.id;
        $scope.orgList = objData.organization;
        !$scope.orgList ? $scope.getCurrency($scope.donate.id, 0) : '';
        !$scope.orgList ? $scope.donate.paymore[0].organization_id = $scope.donate.id : ''; // $scope.getCurrency()
        // 
  ...
  var controller = ['$scope', '$ionicModal', 'APIService', '$ionicLoading', 'ionicToast', 'IMAGE_PATH', function ($scope, $ionicModal, APIService, $ionicLoading, ionicToast, IMAGE_PATH) {
    ...
    function groupByCurrency(response) {
      ...
      response.forEach(function (item) {
        ...
        if (existing.length) {
          ...
          if (typeof item.amount == 'string' || typeof item.amount == 'number') item.amount = [item.amount];
          output.push(item);
        }
      });
      output.forEach(function (element, index) {
        var amount = 0;
        element.amount.forEach(function (el) {
          amount += parseInt(el);
        });
    ...
    socket.on('message', function (response) {
      ...
      var type = getType(response.donatable_type);

      if (JSON.parse($scope.items).id == response.orginization_id && JSON.parse($scope.items).type == type) {
        if ($scope.objData.length && $scope.objData.find(function (x) {
          return x.user.id == response.user.id;
        })) {
          $scope.objData.find(function (x) {
            return x.user.id == response.user.id;
    ...
    socket.on('disconnect', function () {}); //socket end
  }];
  return {
    restrict: 'E',
    // replace: true,
      ...
      $scope.$watch('objectToInject', function (value) {
        ...
        if (value) {
          ...
          $scope.Obj.invoke = function () {
            $scope.getDonarList($scope.$eval($scope.items)); //Do something
          };
        }
      });
    }
...
.directive("attachment", function () {
  var controller = ['$scope', 'APIService', '$stateParams', '$ionicLoading', 'ionicToast', 'BASE_URL', 'Upload', '$ionicActionSheet', 'ATTACH_PATH', '$ionicSlideBoxDelegate', '$ionicModal', function ($scope, APIService, $stateParams, $ionicLoading, ionicToast, BASE_URL, Upload, $ionicActionSheet, ATTACH_PATH, $ionicSlideBoxDelegate, $ionicModal) {
    $scope.attachments = []; // $scope.isLabel = true;
    ...
    $scope.log = ''; // $scope.filepathdsd='https://www.w3schools.com/tags/horse.ogg';
            ...
            if (!file.$error) {
              ...
              Upload.upload({
                ...
                if (files[indexer].type.includes('application/pdf')) {
                  ...
                  orignalfile = [ATTACH_PATH + resp.data.filename, 'img']; // orignalfile = [files[indexer], 'img'];
                }
  ...
  return {
    ...
    // replace: true,
    link: function link($scope, element, attrs) {// scope.$watch(function () {
...
angular.module('starter').factory('genericModalService', ModalService);